...
build_arches:
	${MAKE} arch IOS_ARCH=arm64 IOS_PLATFORM=MacOSX IOS_HOST=arm-apple-darwin IOS_TARGET=arm64-apple-macos12.0 IOS_ARCH_DIR=arm64-macos IOS_VERSION_OPTIONS=""
...
IOS_VERSION_OPTIONS=-mios-version-min=17.0 
...
CXXFLAGS =-target ${IOS_TARGET} -stdlib=libc++ -std=c++11 -isysroot ${IOS_SDK} -I${IOS_SDK}/usr/include -I${INCLUDEDIR} -I${UTHASHDIR} ${IOS_VERSION_OPTIONS} -Os -fembed-bitcode
